...
static FFmpegProgressCB g_progressCb = NULL;
static FFmpegErrorCB g_errorCb = NULL;
int64_t g_taskId = 0;
...
static void ffmpeg_cleanup(int ret)
{
    ...
    // AS by lvaj 2020-02-12 for Exception Exit
    if (ret != 0) {
        g_errorCb(g_taskId);
    }
    // AE by lvaj 2020-02-12 for Exception Exit
}
...
int ffmpeg_main(int argc, char **argv, FFmpegLogCB cbLog, FFmpegErrorCB cbErr, FFmpegProgressCB cbProgress, int64_t taskId)
{
    ...
    EXIT_PROGRAM_N1(received_nb_signals ? 255 : main_return_code);
