...
// AS by lvaj 2020-02-13 for exit_program
#define MATCH_PER_STREAM_OPT_NULL(name, type, outvar, fmtctx, st)\
{\
    int i, ret;\
    for (i = 0; i < o->nb_ ## name; i++) {\
        char *spec = o->name[i].specifier;\
...
#define MATCH_PER_STREAM_OPT_N1(name, type, outvar, fmtctx, st)\
{\
    int i, ret;\
    for (i = 0; i < o->nb_ ## name; i++) {\
        char *spec = o->name[i].specifier;\
...
// AE by lvaj 2020-02-13 for exit_program
...
static AVCodec *choose_decoder(OptionsContext *o, AVFormatContext *s, AVStream *st)
{
    ...
    MATCH_PER_STREAM_OPT_NULL(codec_names, str, codec_name, s, st);
    if (codec_name) {
...
static int choose_encoder(OptionsContext *o, AVFormatContext *s, OutputStream *ost)
{
    ...
    if (type == AVMEDIA_TYPE_VIDEO || type == AVMEDIA_TYPE_AUDIO || type == AVMEDIA_TYPE_SUBTITLE) {
        MATCH_PER_STREAM_OPT_N1(codec_names, str, codec_name, s, ost->st);
        if (!codec_name) {
...
static OutputStream *new_output_stream(OptionsContext *o, AVFormatContext *oc, enum AVMediaType type, int source_index)
{
    ...
    if (ost->enc) {
        ...
        MATCH_PER_STREAM_OPT_NULL(presets, str, preset, oc, st);
    ...
    MATCH_PER_STREAM_OPT_NULL(time_bases, str, time_base, oc, st);
    if (time_base) {
        AVRational q;
    ...
    MATCH_PER_STREAM_OPT_NULL(enc_time_bases, str, time_base, oc, st);
    if (time_base) {
        AVRational q;
        if (av_parse_ratio(&q, time_base, INT_MAX, 0, NULL) < 0 ||
            q.den <= 0) {
    ...
    MATCH_PER_STREAM_OPT_NULL(max_frames, i64, ost->max_frames, oc, st);
    ...
    MATCH_PER_STREAM_OPT_NULL(copy_prior_start, i, ost->copy_prior_start, oc ,st);

    MATCH_PER_STREAM_OPT_NULL(bitstream_filters, str, bsfs, oc, st);
    while (bsfs && *bsfs) {
    ...
    MATCH_PER_STREAM_OPT_NULL(codec_tags, str, codec_tag, oc, st);
    if (codec_tag) {
        uint32_t tag = strtol(codec_tag, &next, 0);
        if (*next)
            tag = AV_RL32(codec_tag);
    ...
    MATCH_PER_STREAM_OPT_NULL(qscale, dbl, qscale, oc, st);
    if (qscale >= 0) {
        ost->enc_ctx->flags |= AV_CODEC_FLAG_QSCALE;
        ost->enc_ctx->global_quality = FF_QP2LAMBDA * qscale;
    }

    MATCH_PER_STREAM_OPT_NULL(disposition, str, ost->disposition, oc, st);
    ost->disposition = av_strdup(ost->disposition);

    ost->max_muxing_queue_size = 128;
    MATCH_PER_STREAM_OPT_NULL(max_muxing_queue_size, i, ost->max_muxing_queue_size, oc, st);
...
static OutputStream *new_video_stream(OptionsContext *o, AVFormatContext *oc, int source_index)
{
    ...
    MATCH_PER_STREAM_OPT_NULL(frame_rates, str, frame_rate, oc, st);
    ...
    MATCH_PER_STREAM_OPT_NULL(frame_aspect_ratios, str, frame_aspect_ratio, oc, st);
    ...
    MATCH_PER_STREAM_OPT_NULL(filter_scripts, str, ost->filters_script, oc, st);
    MATCH_PER_STREAM_OPT_NULL(filters,        str, ost->filters,        oc, st);
    if (o->nb_filters > 1)
    ...
    if (!ost->stream_copy) {
        ...
        MATCH_PER_STREAM_OPT_NULL(frame_sizes, str, frame_size, oc, st);
        ...
        MATCH_PER_STREAM_OPT_NULL(frame_pix_fmts, str, frame_pix_fmt, oc, st);
        ...
        MATCH_PER_STREAM_OPT_NULL(intra_matrices, str, intra_matrix, oc, st);
        if (intra_matrix) {
        ...
        MATCH_PER_STREAM_OPT_NULL(chroma_intra_matrices, str, chroma_intra_matrix, oc, st);
        ...
        MATCH_PER_STREAM_OPT_NULL(inter_matrices, str, inter_matrix, oc, st);
        if (inter_matrix) {
        ...
        MATCH_PER_STREAM_OPT_NULL(rc_overrides, str, p, oc, st);
        for (i = 0; p; i++) {
            int start, end, q;
        ...
        /* two pass mode */
        MATCH_PER_STREAM_OPT_NULL(pass, i, do_pass, oc, st);
        if (do_pass) {
            if (do_pass & 1) {
        ...
        MATCH_PER_STREAM_OPT_NULL(passlogfiles, str, ost->logfile_prefix, oc, st);
        ...
        MATCH_PER_STREAM_OPT_NULL(forced_key_frames, str, ost->forced_keyframes, oc, st);
        if (ost->forced_keyframes)
            ost->forced_keyframes = av_strdup(ost->forced_keyframes);

        MATCH_PER_STREAM_OPT_NULL(force_fps, i, ost->force_fps, oc, st);

        ost->top_field_first = -1;
        MATCH_PER_STREAM_OPT_NULL(top_field_first, i, ost->top_field_first, oc, st);
        ...
        MATCH_PER_STREAM_OPT_NULL(copy_initial_nonkeyframes, i, ost->copy_initial_nonkeyframes, oc ,st);
    }
...
static OutputStream *new_audio_stream(OptionsContext *o, AVFormatContext *oc, int source_index)
{
    ...
    if (!ost->stream_copy) {
        ...
        MATCH_PER_STREAM_OPT_NULL(audio_channels, i, audio_enc->channels, oc, st);

        MATCH_PER_STREAM_OPT_NULL(sample_fmts, str, sample_fmt, oc, st);
        if (sample_fmt &&
        ...
        MATCH_PER_STREAM_OPT_NULL(audio_sample_rate, i, audio_enc->sample_rate, oc, st);

        MATCH_PER_STREAM_OPT_NULL(apad, str, ost->apad, oc, st);
